...
class SparseAttention(Attention):
    def __init__(self, attn_module, head_search=False, uniform_search=False):
        super().__init__(attn_module.in_features, attn_module.num_heads, True, attn_module.scale, attn_module.attn_drop.p, attn_module.proj_drop.p)
        self.is_searched = False
        self.num_gates = attn_module.in_features // self.num_heads
        if head_search:
            self.zeta = nn.Parameter(torch.ones(1, 1, self.num_heads, 1))
        elif uniform_search:
            self.zeta = nn.Parameter(torch.ones(1, 1, 1, self.num_gates))
        else:
            self.zeta = nn.Parameter(torch.ones(1, 1, self.num_heads, self.num_gates))
...
class ModuleInjection:
    ...
    def make_searchable_mlp(mlp_module):
        if ModuleInjection.method == 'full':
            return mlp_module
        mlp_module = SparseFeedForward.from_mlp(mlp_module)
...
class SparseFeedForward(FeedForward):
    def __init__(self, mlp_module):
        super().__init__( mlp_module.hidden_dim)
        self.is_searched = False
        self.num_gates = mlp_module.hidden_dim
    ...
    def forward(self, x):
        ...
        return self.w2(F.silu(self.w1(x)) * self.w3(x) * z)
    ...
    def from_mlp(mlp_module):
        mlp_module = SparseFeedForward(mlp_module)
        return mlp_module


class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: ModelArgs, head_search=False, uniform_search=False):
        """
        ...
        if args.slimming:
            self.attention = ModuleInjection.make_searchable_attn(self.attention, head_search, uniform_search)
        ...
        if args.slimming:
            self.feed_forward = ModuleInjection.make_searchable_mlp(self.feed_forward)
