...
def main(args):
    ...
    # Layer pruning based on the computed influences
    # order layers asc
    cosine_layer_distance = [(layer_influence, idx) for idx, layer_influence in enumerate(cosine_layer_influences)]
    cosine_layer_distance.sort(key=lambda x: x[0], reverse=True)
    remaining_layers = cosine_layer_distance[:int(total_layers * args.sparsity_ratio)]
    remaining_layers = [layer_idx for _, layer_idx in remaining_layers]
    pruned_layers = cosine_layer_distance[int(total_layers * args.sparsity_ratio):]
    pruned_layers = [layer_idx for _, layer_idx in pruned_layers]
    print('Pruned Layers: ', pruned_layers)
    model.select_layers(remaining_layers)
